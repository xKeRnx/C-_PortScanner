...
namespace ConsolePortScanner
{
    class MainClass
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                var task = RunPortScanAsync("31.171.152.19", 1194, 1194);
                task.Wait();
            }
            catch (Exception ex)
            {
        ...
        public static async Task RunPortScanAsync(string ip, int portf, int portt)
        {
            // do a specific range
            Console.WriteLine("> Checking ports "+ portf + "-"+ portt + " on "+ ip + "...\n");
            PortScanner cps = new PortScanner(ip, portf, portt);
